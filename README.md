...
| Research | Optional public-web lookup through Wikipedia, DuckDuckGo, and SerpAPI |
| Memory | SQLite stores task history and scoped key-value memory |
...
| `MEMORY_MAX_VALUE_CHARS` | `8000` | Max value size for `!memory set`. |
...
| Variable | Default | Purpose |
| --- | --- | --- |
| `SERPAPI_KEY` | empty | Optional SerpAPI key. Without it, Wikipedia and DuckDuckGo are still used. |
...
Research uses SerpAPI when `SERPAPI_KEY` is configured, plus Wikipedia and DuckDuckGo.
...
11. Run `npm ci` and `npm run verify` in CI.
12. Back up or rotate `longyan-memory.db` according to your data retention policy.
...
SQLite queries use parameters. Memory keys are prefixed according to `MEMORY_SCOPE`, which defaults to per-user isolation.
